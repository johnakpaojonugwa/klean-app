...
import { 
  MapPin, Phone, Clock, MoreVertical, Edit, Trash2, Users, 
  Building, Mail, Briefcase 
} from "lucide-react";
    ...
    <div className="p-5 pb-0 flex justify-between items-start">
      <div className="space-y-1">
        <div className="flex items-center gap-2">
          ...
          <Badge
            className={branch.isActive ? "bg-emerald-50 text-emerald-700 border-emerald-100 hover:bg-emerald-50" : "bg-slate-50 text-slate-600 border-slate-100"}
            variant="outline"
          >
            <div className={`w-1.5 h-1.5 rounded-full mr-1.5 ${branch.isActive ? "bg-emerald-500" : "bg-slate-400"}`} />
            {branch.isActive ? "Online" : "Closed"}
          </Badge>
        </div>
        <h3 className="font-bold text-slate-900 text-xl tracking-tight leading-none pt-1">{branch.name}</h3>
      </div>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-40">
          ...
          <DropdownMenuItem onClick={() => onDelete(branch._id)} className="text-rose-600 focus:text-rose-600 focus:bg-rose-50">
    ...
    <div className="p-5 space-y-4">
      <div className="space-y-2.5 text-sm text-slate-600">
        <div className="flex gap-3 items-start group/info">
          ...
          <span className="leading-snug pt-0.5">
            {typeof branch.address === "object" ? `${branch.address.street}, ${branch.address.city}` : branch.address}
          </span>
        </div>
      ...
      <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
        <div className="flex flex-col">
          <span className="text-[10px] uppercase font-semibold text-slate-400">Manager</span>
          <span className="text-sm font-semibold text-indigo-950">{branch.manager?.fullname || "Unassigned"}</span>
        </div>
        <div className="flex items-center bg-indigo-50 px-3 py-1.5 rounded-xl border border-indigo-100">
          ...
          <span className="text-xs font-bold text-indigo-700">{staffCount || 0} Staff</span>
        </div>
      </div>
    </div>
  </motion.div>
...
export const BranchFormDialog = ({ 
  showForm, setShowForm, editBranches, formData, errors, 
  handleInputChange, handleSave, isEmployeesPending, employees, isSubmitting 
}) => (
    ...
    <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto p-0 border-0 shadow-2xl">
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 p-8 text-white">
        ...
        <p className="text-indigo-100 text-sm mt-2">
          {editBranches ? "Modify branch details and configuration" : "Create a new branch location and assign manager"}
        </p>
      </div>
      <form onSubmit={handleSave} className="p-8 space-y-8">
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-2 pb-2 border-b border-slate-200">
            ...
            <h3 className="text-lg font-semibold text-slate-800">Address Details</h3>
          </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-2 pb-2 border-b border-slate-200">
            ...
            <h3 className="text-lg font-semibold text-slate-800">Contact & Operations</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
        ...
        <div className="flex gap-4 justify-end pt-8 border-t border-slate-200">
          ...
          <Button type="submit" className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white" disabled={isSubmitting}>
            {isSubmitting ? "Saving..." : editBranches ? "Update Branch" : "Create Branch"}
          </Button>
        </div>
      </form>
    </DialogContent>
